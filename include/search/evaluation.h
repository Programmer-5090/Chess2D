...
class Evaluation {
public:
    explicit Evaluation(const BoardState& board) : board(board) {}
    ~Evaluation() = default;
    ...
    int EvaluateTables() const {
        ...
        for (int color = 0; color < 2; ++color) {
            for (int pieceType = PIECE_PAWN; pieceType <= PIECE_KING; ++pieceType) {
                ...
                for (int i = 0; i < pieceList.count(); ++i) {
                    ...
                    const int value = pieceValue + table[sq];
		...
		int EvaluateTable(const std::array<int, 64>& table, int pieceType, int color) {
			int evaluation = 0;
			const PieceList pieceList = board.getPieceList(color, pieceType);
			for (int i = 0; i < pieceList.count(); ++i) {
				evaluation += table[pieceList[i]];
        }
			return evaluation;
    }
    ...
    int MopUpEval() const {
        const int sideToMove = board.getSide();
        const int opponent = sideToMove ^ 1;

        const int myMaterial = getMaterialWithoutKing(sideToMove);
        const int oppMaterial = getMaterialWithoutKing(opponent);
        const int materialDiff = myMaterial - oppMaterial;

        if (materialDiff < 200) return 0;

        const int myKingSq = getKingSquare(sideToMove);
        const int oppKingSq = getKingSquare(opponent);
        if (myKingSq < 0 || oppKingSq < 0) return 0;

        const int oppKingCenterDist = PrecomputedMoveData::getCentreManhattanDistance(oppKingSq);
        const int kingsDist = PrecomputedMoveData::getKingDistance(myKingSq, oppKingSq);

        int bonus = oppKingCenterDist * 10;
        bonus += (14 - kingsDist) * 4;

        return (sideToMove == COLOR_WHITE) ? bonus : -bonus;
    }
